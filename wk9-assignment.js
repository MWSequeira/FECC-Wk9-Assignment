...
function createDeck () {
    let cardDeck = []; // start with an empty array
    let cardVal = 0;
    let numInSuit = 13;
  ...
  // This function won't reshuffle the same deck twice.
  // You have to call a new deck for each game.
function shuffleDeck (array) { 
    let shuffledDeck = array.slice() // copy the array

    for (let i = array.length - 1; i > -1; i--) { // move the shuffle-end from end to beginnng
        let rdmNum = getRandomInt(0, i); // get a random index
        let temp = array[i]; // put the shuffle-end in the temporary array
        shuffledDeck[i] = shuffledDeck[rdmNum]; // add that card to the new array
...
let newCardDeck = createDeck(); // get a new deck
let gameWarCardDeck = shuffleDeck(newCardDeck); // shuffle the deck
// console.log (gameWarCardDeck); // uncomment this line to see all the cards in the shuffled deck.
